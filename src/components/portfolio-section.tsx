            ...
            >
              {item.image_url ? (
                <div className="h-44 overflow-hidden">
                  <Image src={item.image_url} alt={item.title} width={400} height={176} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" unoptimized />
                </div>
              ) : (
              ...
              <div className="mt-6 flex flex-col md:flex-row gap-8">
                ...
                {(selected.image_url || selected.client_logo) && (
                  <div className="md:w-80 shrink-0">
                    {selected.image_url && (
                      <Image src={selected.image_url} alt={selected.title} width={320} height={240} className="w-full rounded-xl" unoptimized />
                    )}
                    {selected.client_logo && !selected.image_url && (
                      <Image src={selected.client_logo} alt={selected.client_name || "Client"} width={320} height={240} className="w-full rounded-xl" unoptimized />
                    )}
                  </div>
                )}
              </div>
